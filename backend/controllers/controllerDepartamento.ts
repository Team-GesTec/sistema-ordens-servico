...
export class ControllerDepartamento {
    ...
    //DELETE
    public deleteDepartamento(req: Request, resp: Response): Response {
        const { id } = req.params;
        const departamentoId = departamentos.findIndex(dep => dep.idDepartamento === Number(id));
        if (departamentoId === -1) {
            return resp.status(404).json({ mensagem: "Departamento não encontrado" });
        }
        departamentos.splice(departamentoId, 1);
        return resp.status(200).json({ mensagem: "Departamento deletado com sucesso" });
    };
}
